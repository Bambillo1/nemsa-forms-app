...
def save_uploaded_file(file):
    ...
    Returns: (stored_filename, original_filename) or None
    """
    try:
        if not file or file.filename.strip() == '':
            return None

        # Secure both filename and extension
        original_filename = file.filename
        filename, extension = os.path.splitext(original_filename)
        secure_name = secure_filename(filename)
        secure_ext = secure_filename(extension.lower())
        
        if not secure_ext:
            logger.warning("File missing extension")
            return None

        # Validate against allowed extensions again
        if secure_ext[1:] not in current_app.config['ALLOWED_EXTENSIONS']:
            logger.warning(f"Invalid file extension: {secure_ext}")
            return None

        # Generate unique filename
        base_name = secure_name or str(uuid.uuid4())
        final_filename = f"{base_name}{secure_ext}"
        ...
        # Prevent overwrites with counter
        counter = 1
        while os.path.exists(os.path.join(upload_folder, final_filename)):
            final_filename = f"{base_name}_{counter}{secure_ext}"
            counter += 1
